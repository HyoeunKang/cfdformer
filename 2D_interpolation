ux_set = 0.05:0.03:0.95;
uy_set = 0.05:0.03:0.95;
...
cOptDef = {   ...
    ...
    'n_y_cell', 200;
    'n_cell',   100;
    'Ux_path',  '2D_TEST_inter/data/Ux/';
    'Uy_path',   '2D_TEST_inter/data/Uy/';
    'P_path',    '2D_TEST_inter/data/Pressure/';
    'Boundary_path', '2D_TEST_inter/data/Boundary/';
    'Grid_path','2D_TEST_inter/data/Grid/',
    'Subd_path', '2D_TEST_inter/data/Subd/'
    };

[got,opt] = parseopt(cOptDef);

step = 1;
num_step = 66;  % 저장파일 이름 맨 앞부분
ux_setting = 1.1;
uy_setting = 1.1;
...
while (ux_setting<11)&&(uy_setting<11)    
    for i = 1:1 % 1set(ux, uy)에 대하여 sample 20개 생성
        clf('reset')
...
%          Boundary 생성 및 저장 (200(세로) x 300(가로))
        ...
        save('2D_TEST_inter/data/Boundary/'+save_s_total+".mat",'Boundary');
...
%         파일 저장
        ...
        filename = '2D_TEST_inter/data/Solve/'+save_s_total+'.csv';
        writetable(output,filename,'Delimiter', ',','QuoteStrings',true);
        
        num_step = num_step +1;
    end
    ux_setting = ux_setting+0.3;
    uy_setting = uy_setting+0.3;
end
